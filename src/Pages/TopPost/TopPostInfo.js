...
const TopPostInfo = ({data}) => {

    const {user} = useContext(AuthContext);

    const {message, image, _id, postLike} = data;

    return (
        <div className="rounded-md shadow-md bg-gray-800 text-gray-100 mx-10">
            <div className="flex items-center justify-between p-3">
                ...
                <Link to={`/mediaDetail/${_id}`}>
                    <button className='btn text-white px-5 py-3 rounded-md' title="Open options" type="button">
                        Details
                    </button>
                </Link>
            </div>
            <img src={image} alt="" className="object-cover object-center w-full h-72 lg:h-[600px] bg-gray-500" />
            <div className="p-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        <button type="button" title="Love post" className="flex items-center justify-center">
                ...
                <div className="flex flex-wrap items-center pt-3 pb-1">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm">Liked by {' '}
                            <span className="font-semibold">Kasem {' '}</span>and {' '}
                ...
                <div className="space-y-3">
                    ...
                    <input type="text" placeholder="  Add a comment..." className="w-full p-2 border-none rounded text-sm pl-0 text-black" />
                </div>
            </div>
        </div>
    );
